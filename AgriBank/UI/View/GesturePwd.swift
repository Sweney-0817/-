...
class GesturePwd: UIView  {

    var m_BankCode: String = ""
    var m_account:String = ""
       
    var delegate:GesturePwdDelegate? = nil
    ...
    @IBAction func clickClosePBtn(_ sender: Any) {
        delegate?.clickGesturePwdCloseBtn(true)
    }
    //關閉圖形驗證 取消
...
extension GesturePwd: UICollectionViewDataSource {
...
extension GesturePwd: UICollectionViewDelegate {
    
}

extension GesturePwd: UICollectionViewDelegateFlowLayout {
...
extension GesturePwd: GestureCollectionViewDelegate {
        ...
        ckpod = SecurityUtility.utility.AES256Encrypt("AFISCCOMTW" + selectedPod.description, "\(SEA1)\(SEA2)\(SEA3)")
            if ckpod == wkpod  {
                //圖形密碼正確
                 ...
                 self.delegate?.clickGesturePwdCloseBtn(true)
                self.delegate?.clickFastLogInBtn(m_BankCode,m_account ,success: 1)
                ...
                if gestureErCntr == "5" {
                    ...
                    SecurityUtility.utility.writeFileByKey("0" + m_account, SetKey: m_BankCode, setEncryptKey: "\(SEA1)\(SEA2)\(SEA3)")
                    SecurityUtility.utility.writeFileByKey("0", SetKey: "GestureEr", setEncryptKey: "\(SEA1)\(SEA2)\(SEA3)")
                      delegate?.clickGesturePwdCloseBtn(true)
                     showMessageAlert(message: "圖形密碼已輸入錯誤6次，請重新設定快速登入!")
                }else{
                showMessageAlert(message: "圖形密碼輸入錯誤!")
                self.delegate?.clickFastLogInBtn(m_BankCode,m_account,success: 0)
                }
        }
    }
