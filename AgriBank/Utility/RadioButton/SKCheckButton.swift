...
//  AgriBank
//
//  Created by ABOT on 2023/6/15.
//  Copyright © 2023 Systex. All rights reserved.
//

import UIKit
@IBDesignable
open class SKcheckButton: UIButton {
    ...
    @IBInspectable public var marginWidth: Double = 5
      @IBInspectable public var leftmarginWidth: Double = 10
    @IBInspectable public var iconSize: Double = 20
    ...
    override open func draw(_ rect: CGRect) {
      if iconView == nil {
        iconView = UIView(frame: CGRect(x: 0.0 + leftmarginWidth, y: self.frame.origin.y/2.0, width: CGFloat(iconSize), height: CGFloat(iconSize)))
        iconView?.clipsToBounds = true
        iconSecondInnerView = UIView(frame: CGRect(x: 0 + leftmarginWidth, y: 0, width: iconSize*0.8, height: iconSize*0.8))
        iconSecondInnerView?.clipsToBounds = true
        iconThirdInnerView = UIView(frame: CGRect(x: 0 + leftmarginWidth, y: 0, width: iconSize*0.5, height: iconSize*0.5))
       ...
       // iconThirdInnerView?.layer.cornerRadius = iconThirdInnerView!.frame.size.width / 2 - 1
        let x = Double(iconView!.frame.size.width) + marginWidth + leftmarginWidth + 5 //+5讓label離遠點
      ...
      //iconView?.frame = CGRect(x: 0.0, y: Double(center.y - iconView!.frame.height/2.0), width: iconSize, height: iconSize)
        iconView?.frame = CGRect(x: 0.0 + leftmarginWidth, y: 0, width: iconSize, height: iconSize)
      ...
      // Set position for text label
        //+5讓label離遠點
      let x = Double(iconView!.frame.size.width) + marginWidth + leftmarginWidth + 5
      let rect = CGRect(x: x, y: -1, width: Double(frame.size.width)-x, height: Double(frame.size.height))
